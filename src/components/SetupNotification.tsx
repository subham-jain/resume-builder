'use client';

import { useState, useEffect } from 'react';

export default function SetupNotification() {
  const [showNotification, setShowNotification] = useState(false);

  useEffect(() => {
    // Check if environment variables are missing
    const hasSupabaseConfig = process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
    const hasGeminiConfig = process.env.GEMINI_API_KEY;
    
    if (!hasSupabaseConfig || !hasGeminiConfig) {
      setShowNotification(true);
    }
  }, []);

  if (!showNotification) return null;

  return (
    <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6">
      <div className="flex">
        <div className="flex-shrink-0">
          <svg className="h-5 w-5 text-yellow-400" viewBox="0 0 20 20" fill="currentColor">
            <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
          </svg>
        </div>
        <div className="ml-3">
          <h3 className="text-sm font-medium text-yellow-800">
            Setup Required
          </h3>
          <div className="mt-2 text-sm text-yellow-700">
            <p>
              To use the full functionality of this resume builder, you need to configure your environment variables.
              Please check the <code className="bg-yellow-100 px-1 rounded">SETUP.md</code> file for detailed instructions.
            </p>
            <div className="mt-3">
              <button
                onClick={() => setShowNotification(false)}
                className="bg-yellow-100 hover:bg-yellow-200 text-yellow-800 px-3 py-1 rounded text-xs font-medium transition-colors"
              >
                Dismiss
              </button>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
